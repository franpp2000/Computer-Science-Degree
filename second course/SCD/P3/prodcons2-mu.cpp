...
{
   static int contador = num_productor * (num_items/np);
...
void funcion_consumidor(int num_consumidor)
{
   ...
   for( unsigned int i=0 ; i < num_items/nc; i++ )
   {
      ...
      MPI_Recv ( &valor_rec, 1, MPI_INT, id_buffer, etiq_c, MPI_COMM_WORLD,&estado );
...
void funcion_buffer()
{
   ...
   for( unsigned int i=0 ; i < num_items*2 ; i++ )
   {
      ...
      if ( num_celdas_ocupadas == 0 )               // si buffer vacío
         tag = etiq_p;       // $~$ solo prod.
      else if ( num_celdas_ocupadas == tam_vector ) // si buffer lleno
         tag = etiq_c;      // $~$ solo cons.
