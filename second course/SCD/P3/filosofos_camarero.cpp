...
{
	int num_filosofos_sentados = 0, peticion, correcto, id_filosofo;
	MPI_Status estado;
	...
	while(true)
	{
		...
		{
			MPI_Recv(&peticion, 1, MPI_INT, id_filosofo, etiq_levantarse, MPI_COMM_WORLD, &estado);
			num_filosofos_sentados--;
		}
		else if(estado.MPI_TAG == etiq_sentarse)
		{
			MPI_Recv(&peticion, 1, MPI_INT, id_filosofo, etiq_sentarse, MPI_COMM_WORLD, &estado);
			num_filosofos_sentados++;
		}	
