...
def age_pos(tam_poblacion, n, m, elementos, m_dist):
    t = 0
    ...
    while t < 100000:
        ...
        #start = time.time()
        intermedia = cruce_age_pos(padres, n_cruces, n, m, elementos, m_dist)
        #end = time.time()
...
for file in contenido:
    ...
    start = time.time()
    sol = age_pos(50, n, m, np.arange(0, int(n), 1, dtype=np.int32), m_dist)
    end = time.time()
