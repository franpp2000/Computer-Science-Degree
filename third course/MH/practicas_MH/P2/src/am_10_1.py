...
def am_10_1(tam_poblacion, n, m, elementos, m_dist):
    t = 0
    g = 0
    ...
    while t < 100000:
        ...
        if g % 10 == 0:
            #start = time.time()
            poblacion, eval = aplicar_BL(poblacion, n, m, elementos, m_dist)
            #end = time.time()
...
for file in contenido:
    ...
    start = time.time()
    sol = am_10_1(10, n, m, np.arange(0, int(n), 1, dtype=np.int32), m_dist)
    end = time.time()
